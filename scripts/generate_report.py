...
def _add_page_footer(canvas, doc):
    ...
    canvas.drawCentredString(
        letter[0] / 2, 0.4 * inch,
        f"maxym-ai-ads v1.5  |  Page {doc.page}  |  {datetime.now().strftime('%B %d, %Y')}"
    )
    canvas.restoreState()
...
def build_pdf(data: dict, output_path: str, brand_name: str = ""):
    ...
    elements.append(Paragraph(
        f"Generated {datetime.now().strftime('%B %d, %Y')}  |  Powered by maxym-ai-ads v1.5",
        styles["RSubtitle"],
    ))
    ...
    elements.append(Spacer(1, 6))
    elements.append(Paragraph(
        "Report generated by maxym-ai-ads v1.5  |  "
        "https://github.com/MAXYMIZE-BUSINESS/maxym-ai-ads",
        styles["RFooter"],
    ))
